...
from itcl_inference_engine.util.quantization import (
...
class FullyConnected(ILayer):
    ...
    def __init__(
        ...
        self.__bias:np.ndarray = np.array(bias)  #np.expand_dims(bias, axis=1) # expand for batch matmul
    ...
    def build(
        ...
        return FullyConnected(input_zp, weights, bias, quant_mult_params, output_zp, output_dtype = output_dtype)
    ...
    def __calculate_accumulator(self, input: np.ndarray):
        ...
        input = input.astype(np.int64) - np.int64(z1)
        ...
        if input.ndim == 2:
            ...
            W = weights_matrix.T
            mmul_res = np.matmul(input,W)
        else:
            mmul_res = np.matmul(weights_matrix, input) 
        ...
        for weight_vector, bias in zip(weights_matrix, bias_vector):
            ...
            for input_value, weight in zip(input, weight_vector):
    ...
    def __multiply_by_quantized_multiplier(self, accumulator: np.ndarray):
        ...
        round = self.__round.astype(np.int64)
        res = accumulator * quantized_multiplier + round
    ...
    def infer(self, input: np.ndarray):
        ...
        assert input.ndim <= 2, f"Input Dimension is greater than 2"

        accumulator = self.__calculate_accumulator(input)
        ...
        # Cast back to INT8
        output = np.clip(result, self.__q_output.min_value(), self.__q_output.max_value())
        return output
...
class FullyConnectedQuantizedMultiplier:
    ...
    def __adjust_mult(self, quantized_multiplier: np.float32, shift: np.int64):
        ...
        shift = int32_addition(31, -shift)

        round = np.int64(1 << (shift - 1))

        return quantized_multiplier, shift, round
    ...
    def __build(
        ...
        return self.__adjust_mult(quantized_multiplier, shift)
