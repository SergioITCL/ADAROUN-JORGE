...
from itcl_inference_engine.layers.onnx.buildOperator import build_operator
from itcl_inference_engine.layers.tflite.buildLayer import build_layer
from itcl_quantization.json.specification import JsonNetwork, Operator
from itcl_inference_engine.util.quantization import from_int8, to_int8
...
class SequentialNetwork(Network):
    ...
    def __init__(
        ...
        self.input_scale, self.input_zero_point = input_node["scale"], input_node["zero_point"]
    ...
    def from_json_file(cls, model_path: str):
        ...
        with open(model_path) as f:
            ...
            return cls(net)
    def infer(
        self, input: np.ndarray, isInputQuant=False, verbose=False, isBatch=False
    ) -> np.ndarray:
        """Inference Method
        ...
        if not self.has_quant_layers:
            ...
            input = to_int8(input, self.input_zero_point or 0, self.input_scale or 1)

        for i, layer in enumerate(self.layers):
            
            input = res = layer.infer(input)
            self._intermediate_results.append(input)
            if verbose:
