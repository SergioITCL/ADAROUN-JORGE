...
class TestFullWindModel:
    ...
    def test_model(self):
        ...
        expected_intermediate_tensors = self.load_expected(self.wd/"intermediate_tensors.npy")
        ...
        for expected, pred in zip(expected_intermediate_tensors[:-1], pred_intermediate_tensors[:-1]):
            np.testing.assert_equal(pred, expected)
        np.testing.assert_almost_equal(expected_intermediate_tensors[-1], pred_intermediate_tensors[-1])
        print(expected_intermediate_tensors)
        print("=========")
        print(pred_intermediate_tensors)
    ...
    def test_model_500(self):
        ...
        expected_intermediate_tensors = self.load_expected(self.wd/"intermediate_tensors_500.npy")
        pred_intermediate_tensors = net._intermediate_results
        ...
        print("mse", np.square(expected_intermediate_tensors[-1] - pred_intermediate_tensors[-1]).mean())