...
OVERRIDE_FUNCTIONS: Dict[str, Callable[[np.ndarray], np.ndarray]] = {
    "SOFTMAX": lambda x: x,
}
...
def quantize_activation_node(
    ...
    input_node: Node,
    trim_dist: Callable[[Distribution], Distribution],
    out_params: Tuple[float, int] | None = None,
) -> Tuple[Operator | None, np.ndarray]:
    """
    ...
    if activation_name in LUT_FUNCTIONS:
        activation_name += "LUT"
